...
// envelope over the JPEG's base64 (docs/data/portrait.jpg.enc), so it is only
// visible after login like every other piece of content. Fails silently — the
...
export async function portraitDataUrl() {
  ...
  const { text } = await readText('data/portrait.jpg.enc');
  const b64 = await decryptString(getPassword(), JSON.parse(text));
  cached = 'data:image/jpeg;base64,' + b64;
  return cached;
}
